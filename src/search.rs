...
impl Graph {
    ...
    pub fn search(&self, hash_in: Int, hash_out: Int) -> Option<()> {
        // Get our starting points!
        let mut queue_in: BinaryHeap<_> = match self.edges_in.get(&hash_in) {
            Some(edges) => edges.iter().map(|e| Reverse(State::new(e.cost, e, None))).collect(),
            None => BinaryHeap::new(),
        };
            ...
            if !queue_in.is_empty() && queue_in.len() < queue_out.len() {
                ...
                if let Some(edges) = self.edges_in.get(&state.edge.hash_out) {
                    ...
                    for edge in edges {
                        ...
                        queue_in.push(
                            Reverse(State::new(1, edge, Some(Rc::clone(&state_rc))))
                        )
                    }
                }
